    ...
    float3* d_results, int k, float radius)
{
    ...
    cukd::FixedCandidateList<FIXED_K> result(radius); // Fixed at 16, for generalization make template
...
void run_main() {
  ...
  for (int r=0;r<10;r++) {
    ...
    {
      int N = 100000000 + int(20000 * rng(gen));
      for (int i=0;i<N;i++) {
    ...
    {
      int N = 1240000 + int(20000 * rng(gen));
      for (int i=0;i<N;i++) {
